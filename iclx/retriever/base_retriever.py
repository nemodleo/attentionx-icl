...
class BaseRetriever:
    ...
    def generate_ice(self, idx_list: List[int], ice_template: Optional[PromptTemplate] = None, pseudo_gt: Optional[str] = None, shuffle: Optional[str] = None) -> str:
        generated_ice_list = []
        ...
        for idx in idx_list:
            if ice_template is None:
                ...
                generated_ice_list.append(
                    ice_template.generate_ice_item(self.index_ds[idx], self.index_ds[idx][dr.output_column], use_ordering=self.use_ordering, shuffle=shuffle))
